...
// input index really input x*2pi
double func_2d (double x) {
   double out = (-4.0)*sin(2.0*x*2.0*M_PI) + exp(-1.0*pow(x*2.0*M_PI - M_PI, 4))*(16.0*pow((x*2.0*M_PI - M_PI), 6) -12.0*pow((x*2.0*M_PI - M_PI), 2));
//   double out = (-1.0)*sin(x*2.0*M_PI);
   return out;
}
...
long double complex  approx_2d (long double complex * coeff, double in, int N) {
   int i;
   long double complex out = 0;
   for (i = 0; i < N; i = i + 1) {
     out = out + euler((i - N/2.0)*in)*coeff[i]*(i-N/2.0)*(-1)*(i-N/2.0); 
   } 
   return out;
}
...
int main () {
   ...
   long double complex kk = euler(3);
   printf ("%1f, %1fi\n", crealf(kk), cimagf(kk));
 ...
 //        printf("%1f \n", -1.0*seed[i]*j*(2.0/8.0));
   ...
   long double complex test [N];
   printf("start of test output \n", i);
   for (i = 0; i < N; i = i + 1) {
        test [i] = approx(A, i/N_comp, N);
